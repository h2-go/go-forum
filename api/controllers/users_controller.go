...
import (
	...
	"github.com/iryek219/go-forum/api/auth"
	"github.com/iryek219/go-forum/api/models"
	"github.com/iryek219/go-forum/api/security"
	"github.com/iryek219/go-forum/api/utils/fileformat"
	"github.com/iryek219/go-forum/api/utils/formaterror"
)
...
func SaveProfileImage(s *session.Session, file *multipart.FileHeader) (string, error) {
 	...
 	_, err = s3.New(s).PutObject(&s3.PutObjectInput{
 		...
 		Body:                      	fileBytes,
 		Bucket:                    	aws.String("iryek219-go-admin"),
...
func (server *Server) UpdateAvatar(c *gin.Context) {
	...
	s, err := session.NewSession(
		&aws.Config{
			...
			Credentials: credentials.NewStaticCredentials(
				os.Getenv("AWS_KEY"), os.Getenv("AWS_SECRET"), os.Getenv("AWS_TOKEN"),),
	})
	if err != nil {
